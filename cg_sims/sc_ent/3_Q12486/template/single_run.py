...
def calc_Q(current_cor):
    """
    Compute fraction of native contacts for a coordinate set. Uses secondary-structure
    residues and distance cutoff (sdist * native distance).
    """
    ...
    for i in range(len(current_cor) - 4):
        ...
        for j in range(i + 4, len(current_cor)):
            ...
            if native_contact_map[i][j] == 1:
                dist = pow(
                    pow(current_cor[i][0] - current_cor[j][0], 2)
                    + pow(current_cor[i][1] - current_cor[j][1], 2)
                    + pow(current_cor[i][2] - current_cor[j][2], 2),
                    0.5,
                )
...
###### Q mod filter ######
def calc_Q_mod(Q_ts):
    """Return modal Q value (bin with highest count in 0–1.02, step 0.02)."""
...
sdist = (
    1.2  # multiple factor of native distance to determine native contact in trajectory
)
...
native_contact_map = [
    [0 for j in range(len(native_cor))] for i in range(len(native_cor))
]
native_distance_map = [
    [0 for j in range(len(native_cor))] for i in range(len(native_cor))
]
...
for i in range(len(native_cor) - 4):
    for j in range(i + 4, len(native_cor)):
        dist = pow(
            pow(native_cor[i][0] - native_cor[j][0], 2)
            + pow(native_cor[i][1] - native_cor[j][1], 2)
            + pow(native_cor[i][2] - native_cor[j][2], 2),
            0.5,
        )
        if dist <= dist_cutoff:
...
if restart:
    ...
    simulation.reporters.append(
        DCDReporter(outname + "_prod.dcd", nstxout, append=True)
    )
    simulation.reporters.append(
        StateDataReporter(
            f"{outname}.log",
    ...
    simulation.reporters = []
    simulation.reporters.append(
        DCDReporter(outname + "_heating.dcd", nstxout, append=False)
    )
    simulation.reporters.append(
        StateDataReporter(
            f"{outname}_heating.log",
            nstlog,
            step=True,
            time=True,
            potentialEnergy=True,
    ...
    last_heating_positions = simulation.context.getState(
        getPositions=True
    ).getPositions()
    ...
    simulation.reporters.append(CheckpointReporter(cpfile, nstxout))
    simulation.reporters.append(
        DCDReporter(outname + "_prod.dcd", nstxout, append=False)
    )
    simulation.reporters.append(
        StateDataReporter(
            f"{outname}.log",
